...
class FastAPIScaleEndpointTests(unittest.TestCase):
    ...
    def test_scales_catalog_http(self) -> None:
        ...
        self.assertIn("isi7", payload)
        self.assertIn("who5", payload)
        self.assertIn("phq15", payload)
        self.assertIn("question_bank", payload["phq9"])
        self.assertEqual(len(payload["scl90"]["question_bank"]["questions"]), 90)
        self.assertIn("display_name_i18n", payload["who5"])
        self.assertIn("disclaimer_i18n", payload["who5"])
    ...
    def test_isi7_scale_score_http(self) -> None:
        response = self.client.post(
            "/api/scales/score",
            json={
                "scale_id": "isi7",
        ...
        self.assertEqual(payload["severity"], "subthreshold")
